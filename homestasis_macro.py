...
st.set_page_config(layout="wide", page_title="Homeostasis Económica — LaTeX Corregido")
...
# --- 4. Desarrollo matemático del IEM (¡CORREGIDO!) ---
st.subheader("📘 Fundamento matemático del IEM")
st.markdown(r"""
El **Índice de Equilibrio Macro (IEM)** se define como:

$$
\text{IEM}_t = \underbrace{\sum_{s=t-k}^{t} \left( |y_s - \bar{y}_s| + |\pi_s - \bar{\pi}_s| + |i_s - \bar{i}_s| \right)}_{\text{Suma acumulada de desviaciones } (T)} + \kappa - \left( |y_t - \bar{y}_t| + |\pi_t - \bar{\pi}_t| + |i_t - \bar{i}_t| \right)
$$
...
# --- 6. Desarrollo matemático del SVAR (¡CORREGIDO!) ---
...
> ✅ **Interpretación**: Captura causalidad en el sentido de **Granger estructural**.
""")
...
for _ in range(4):
    ...
    pred_value = pred.item()  # ✅ Corrección clave
